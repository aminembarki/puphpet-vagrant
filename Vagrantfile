...
unless Vagrant.has_plugin?("vagrant-vbguest")
  raise '"vagrant-vbguest" plugin is not installed! see https://github.com/dotless-de/vagrant-vbguest.'
end

unless Vagrant.has_plugin?("vagrant-hostsupdater")
  raise '"vagrant-hostsupdater" plugin is not installed! see https://github.com/cogitatio/vagrant-hostsupdater.'
end
...
#Vagrant::Hostsupdater
  ...
  #retrieve apache hosts from vhosts
  dataApache = configValues['apache']

  if dataApache['install'] == '1' && !dataApache['vhosts'].nil? && !dataApache['vhosts'].empty?
    dataApache['vhosts'].each do |key, value|
	  if !value['servername'].empty?
        config.hostsupdater.aliases << value['servername']
	  end
	  if !value['serveraliases'].nil? && !value['serveraliases'].empty?
        value['serveraliases'].each do |host|
  ...
  dataNginx = configValues['nginx']

  #retrieve nginx hosts from vhosts
  if dataNginx['install'] == '1' && !dataNginx['vhosts'].nil? && !dataNginx['vhosts'].empty?
    dataNginx['vhosts'].each do |key, value|
	  if !value['server_name'].empty?
        config.hostsupdater.aliases << value['server_name']
	  end
	  if !value['server_aliases'].nil? && !value['server_aliases'].empty?
        value['serveraliases'].each do |host|
          config.hostsupdater.aliases << host
        end
	  end
  ...
  config.hostsupdater.aliases = config.hostsupdater.aliases.uniq
end
